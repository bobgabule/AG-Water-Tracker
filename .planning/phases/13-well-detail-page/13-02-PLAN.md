...
const { position } = useGeolocation(); // existing hook from the project
...
const proximityInfo = useMemo(() => {
  if (!position || !currentWell?.location) return null;
...
**Important notes:**
- `useGeolocation` is the existing hook at `src/hooks/useGeolocation.ts`. Check its return type -- it may return `{ position, error, loading }` or similar. Read the hook file to confirm the correct property names. If it uses `autoRequest: false` by default, that's fine -- we only display proximity when position is available. Do NOT auto-request location from the well detail page.
- If `useGeolocation` does not exist as a standalone hook, the geolocation data may be in a different provider. Check the codebase. As a fallback, use `navigator.geolocation.getCurrentPosition` in a `useEffect` with a ref, but prefer the existing hook.
- The proximity indicator only shows when GPS position is available (not null). It does not prompt for location permission.
  </action>
  <verify>
1. `npx tsc -b --noEmit` passes with no errors
