...
3. Export function `useWellReadings(wellId: string | null)`:
   ...
   - Query: `SELECT id, well_id, value, recorded_by, recorded_at, gps_latitude, gps_longitude, is_in_range, notes, created_at, updated_at FROM readings WHERE well_id = ? ORDER BY recorded_at DESC`
   - Pass `[wellId]` as params when wellId is non-null
   - Use `useQuery<ReadingRow>(query, params)` from `@powersync/react`
   - Memoize with `useMemo`: map rows to `Reading[]`, converting `is_in_range` from `row.is_in_range === 1` to boolean
...
3. Export function `useWellAllocations(wellId: string | null)`:
   - Guard: if `wellId` is null, use `'SELECT NULL WHERE 0'`
   - Query: `SELECT id, well_id, period_start, period_end, allocated_af, used_af, is_manual_override, notes, created_at, updated_at FROM allocations WHERE well_id = ? ORDER BY period_start DESC`
   - Memoize with `useMemo`, converting `is_manual_override` from integer to boolean
