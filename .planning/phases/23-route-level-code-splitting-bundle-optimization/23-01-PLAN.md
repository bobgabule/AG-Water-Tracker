...
**Mapbox CSS loading** — Verify and preserve lazy-loading:

Per CONTEXT.md decision ("Lazy-load Mapbox CSS with the map chunk, import in map wrapper component, not globally"): Mapbox CSS is already correctly imported in `src/components/MapView.tsx` (`import 'mapbox-gl/dist/mapbox-gl.css'`), NOT in any global location (index.css, main.tsx, or index.html). This is already lazy-loaded since MapView is part of the DashboardPage chunk which is code-split. **No changes needed** — just verify during implementation that no global Mapbox CSS import exists in `src/index.css` or `src/main.tsx`, and do NOT add one.
